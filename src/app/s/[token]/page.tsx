...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: project ? `${project.name} · geteilt` : "Link ungültig", robots: { index: false, follow: false } };
}
    ...
    <div className="mx-auto w-full max-w-6xl space-y-6 px-3 py-6 sm:px-5 sm:py-8">
      <header className="glass flex flex-wrap items-center justify-between gap-3 px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          <span className="chip !py-0.5 text-xs"><Eye size={13} /> Nur lesen</span>
          {isMember ? (
            <Link href={`/projects/${project.id}`} className="btn btn-primary btn-sm">Zum Projekt <ArrowRight size={14} /></Link>
          ) : !user ? (
            <Link href={loginHref} className="btn btn-sm"><LogIn size={14} /> Anmelden</Link>
          ) : null}
        </div>
      </header>
      ...
      <section className="glass relative overflow-hidden p-6 sm:p-8">
        ...
        <div className="mt-5 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm">
          ...
          <span className="text-muted">Priorität: {priorityLabel(project.priority)}</span>
          <span className="inline-flex items-center gap-1.5 text-muted"><UserRound size={14} /> geteilt von {displayNameOf(project.owner)}</span>
      ...
      {!user && (
        <p className="glass px-5 py-4 text-sm text-muted">
          Du möchtest mitarbeiten? <Link href={loginHref} className="text-accent-ink hover:underline">Melde dich an</Link> und frag den Besitzer nach Zugriff.
        </p>
      )}
      ...
      <section className="glass p-6 sm:p-8" aria-labelledby="shared-tasks">
        <h2 id="shared-tasks" className="mb-5 flex items-center gap-2 text-lg font-semibold">
          Aufgaben
          {project.tasks.length > 0 && <span className="rounded-full bg-fg/10 px-2 text-xs font-normal tabular-nums text-muted">{done}/{project.tasks.length} erledigt</span>}
        </h2>
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {TASK_STATUSES.map((s) => {
            const list = tasks.filter((t) => t.status === s.value);
            return (
              <div key={s.value} className="flex min-w-0 flex-col rounded-2xl border bg-bg/25 p-2.5" role="group" aria-label={s.label}>
                ...
                {list.length === 0 ? (
                  <p className="rounded-xl border border-dashed px-3 py-6 text-center text-xs text-muted">Keine Aufgaben</p>
                ) : (
                      ...
                      <li key={t.id} className="glass !rounded-xl p-2.5 text-sm">
                        ...
                        {(t.dueDate || t.labels.length > 0 || t.issueUrl) && (
                          <div className="mt-1.5 flex flex-wrap items-center gap-1.5 text-[11px] text-muted">
                            ...
                            {t.dueDate && <span className="rounded-md border px-1.5">{formatDue(dayKey(t.dueDate), today)}</span>}
      ...
      <footer className="pb-4 text-center text-xs text-muted">
        Geteilt mit {config.appName} · Notizen sind nur für Mitglieder sichtbar
      </footer>
    </div>
  );
}
