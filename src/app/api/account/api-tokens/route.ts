...
export const POST = route(async (req) => {
  ...
  const { name, projects, lifetime } = await readBody(req, apiTokenCreateSchema, { maxBytes: 4096 });
  ...
  const row = await db.apiToken.create({
    data: {
      ...
      tokenHash: hash,
      hint,
      ...(projects ? { projectScoped: true, projectIds: granted.map((p) => p.id), scope: "tasks" } : {}),
