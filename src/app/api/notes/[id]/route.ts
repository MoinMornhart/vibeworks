...
import { findOwnNote, noteLabel, serializeNote, touchProject } from "@/lib/notes";
...
export const PATCH = route<Params>(async (req, { params }) => {
  ...
  const current = await findOwnNote(user.id, id);
  if (!current) throw notFound("Notiz nicht gefunden");
...
export const DELETE = route<Params>(async (_req, { params }) => {
  const user = await requireApiUser();
  const { id } = await params;
  const note = await findOwnNote(user.id, id);
