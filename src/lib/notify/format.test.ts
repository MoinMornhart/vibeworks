...
describe("Anlässe", () => {
  it("fehlende Schalter sind an, ausdrücklich ausgeschaltete aus", () => {
    expect(eventsOf(null)).toEqual({ taskDue: true, accessRequest: true, issueClosed: true, ciFailed: true, checkAlert: true, gitFailed: true, siteDown: true, renewal: true, suggestions: true, updated: true });
