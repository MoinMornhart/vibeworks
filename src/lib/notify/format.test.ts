...
describe("Anlässe", () => {
  it("fehlende Schalter sind an, ausdrücklich ausgeschaltete aus", () => {
    expect(eventsOf(null)).toEqual({ taskDue: true, assigned: true, fork: true, accessRequest: true, issueClosed: true, ciFailed: true, checkAlert: true, appError: true, community: true, team: true, gitFailed: true, siteDown: true, renewal: true, suggestions: true, weeklyReport: true, issueComment: true, issueImported: true, loginAlert: true, passwordAge: true, passwordAsk: true, news: true, updated: true });
