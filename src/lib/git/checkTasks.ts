  ...
  for (const plan of plans) {
    ...
    if (action === "create") {
      const created = await db.task.create({
        data: { projectId, title, description, labels, autoKey: plan.key, assignee: "Claude", status: "TODO", position: await nextTaskPosition(db, projectId, "TODO"), createdVia: "auto" },
      });
