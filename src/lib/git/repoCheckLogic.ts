...
export interface CheckReport {
  ...
  tools: { gitleaks: boolean; osv: boolean; semgrep: boolean; todos: boolean };
  secrets: CheckSecret[];
...
const rows = (v: unknown): Row[] => (Array.isArray(v) ? v.filter((x): x is Row => Boolean(x) && typeof x === "object").slice(0, MAX) : []);
...
export function parseCheckReport(raw: unknown): CheckReport {
  ...
  const findings = rows(r.findings).map((x) => ({ file: str(x.file, 500), line: num(x.line), rule: str(x.rule, 200), severity: str(x.severity, 20), message: str(x.message, 400) }));
  ...
  return {
    ...
    tools: { gitleaks: bool(tools.gitleaks), osv: bool(tools.osv), semgrep: bool(tools.semgrep), todos: bool(tools.todos) },
    secrets,
    vulnerabilities,
    findings,
    todos,
