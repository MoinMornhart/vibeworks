    ...
    <article className={cn("glass group !rounded-xl p-2.5", done && "opacity-75", overlay && "rotate-1 shadow-2xl ring-2 ring-accent/60")}>
      ...
      {(t.dueDate || t.recurrence || t.description || t.labels.length > 0 || t.issueNumber || t.issueError || workers.length > 0 || t.createdByName) && (
        <div className="mt-1.5 flex flex-wrap items-center gap-1.5 pl-[3.25rem] text-muted">
          ...
          {t.createdByName && (
            <span className="inline-flex items-center gap-0.5 text-[11px]" title={tr("card.createdBy", { name: t.createdByName })} data-testid="task-creator">
              <PenLine size={11} /> {t.createdByName}
            </span>
          )}
