...
function ProjectLink({ project }: { project: OverviewTask["project"] }) {
  return (
    <Link href={`/projects/${project.id}`} className="mt-0.5 inline-flex max-w-full items-center gap-1.5 truncate text-xs text-muted hover:text-fg">
      <span className="h-2 w-2 shrink-0 rounded-full" style={{ background: accentGradient(project.accent) }} />
      {project.name}
    </Link>
  );
}
...
export function TodayView({
  ...
  async function remove(task: OverviewTask) {
    setError(null);
    try {
      await api(`/api/today?taskId=${encodeURIComponent(task.id)}`, { method: "DELETE" });
  ...
  return (
    <div className="fade-in mx-auto max-w-3xl space-y-6">
      <header>
        <p className="flex items-center gap-1.5 text-sm font-medium text-accent-ink"><Sun size={15} /> {t("title")}</p>
        <h1 className="text-3xl font-bold tracking-tight">{heading}</h1>
        <p className="mt-1 text-muted">{focus.length ? t("summary", { done, n: focus.length }) : t("emptyHint")}</p>
        {focus.length > 0 && (
          <div className="mt-3 h-2 overflow-hidden rounded-full bg-fg/10" role="progressbar" aria-valuenow={done} aria-valuemin={0} aria-valuemax={focus.length}>
            <div className="h-full rounded-full bg-accent transition-[width] duration-500" style={{ width: `${(done / focus.length) * 100}%` }} />
          </div>
        )}
      </header>
      ...
      <section aria-labelledby="focus-heading" className="glass p-5">
        ...
        <ul className="space-y-2">
          {focus.map((task) => {
            ...
            return (
              <li key={task.id} data-testid="focus-task" className={cn("flex items-center gap-3 rounded-xl border px-3 py-2.5", isDone && "opacity-70")}>
                <button
                  type="button"
                  role="checkbox"
                  aria-checked={isDone}
                  aria-label={isDone ? tt("card.reopen", { title: task.title }) : tt("card.complete", { title: task.title })}
                  onClick={() => void toggle(task)}
                  className={cn("flex h-6 w-6 shrink-0 items-center justify-center rounded-full border transition", isDone ? "border-emerald-400 bg-emerald-400 text-black" : "hover:border-emerald-400")}
                >
                  {isDone && <Check size={14} strokeWidth={3} />}
                </button>
                <div className="min-w-0 flex-1">
                  <p className={cn("truncate", isDone && "text-muted line-through")}>{task.title}</p>
      ...
      <section aria-labelledby="suggest-heading" className="glass p-5">
        <h2 id="suggest-heading" className="text-sm font-semibold uppercase tracking-wider text-muted">{t("suggestions")}</h2>
        ...
        {suggestions.length === 0 ? (
          ...
          <ul className="space-y-2">
            {suggestions.map((task) => (
              <li key={task.id} data-testid="suggestion" className="flex items-center gap-3 rounded-xl border px-3 py-2">
                <div className="min-w-0 flex-1">
