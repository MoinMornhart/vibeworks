...
// Live-Vorschau aus dem Editor) aus dem ThemeProvider.
...
function useIsDark(theme: Theme): boolean {
  ...
  return luminance(palette.bg) < 0.4;
}
...
function CanvasLayer({ preset, theme, dark, reduced }: { preset: CanvasPreset; theme: Theme; dark: boolean; reduced: boolean }) {
  const ref = useRef<HTMLCanvasElement>(null);
  const colors = presetColors(theme);
  const { speed, intensity } = theme.background.preset;
  const key = `${preset}|${colors.join()}|${speed}|${intensity}|${dark}|${reduced}`;

  useEffect(() => {
    const canvas = ref.current;
    const ctx = canvas?.getContext("2d");
    if (!canvas || !ctx) return;
    const engine = CANVAS_ENGINES[preset](ctx, {
      colors,
      speed: reduced ? 0 : speed / 100,
    ...
    const onVisibility = () => {
      ...
      if (!document.hidden && !reduced && speed > 0) {
        last = performance.now();
        raf = requestAnimationFrame(loop);
      }
    };
    const onPointer = (e: PointerEvent) => engine.pointer?.(e.clientX, e.clientY);
...
export function BackgroundView({ theme, contained = false }: { theme: Theme; contained?: boolean }) {
  ...
  const paused = bg.type === "preset" ? bg.preset.speed === 0 : bg.type === "gradient" ? !bg.gradient.animate : true;
  ...
  if (bg.type === "preset") {
    ...
    if (id in CANVAS_ENGINES) {
      layer = <div style={{ opacity: Math.min(1, opacity + 0.25) }}><CanvasLayer preset={id as CanvasPreset} theme={theme} dark={dark} reduced={reduced} /></div>;
    } else if (id === "nebula") {
    ...
    layer = (
      <div
        className={`vw-gradient is-${g.kind}${g.animate ? " is-animated" : ""}`}
  ...
  return (
    <div className={`vw-bg${paused || reduced ? " vw-paused" : ""}`} style={style} aria-hidden>
      {layer}
